...
#使用滑动平均模型的好处：
#在使用梯度下降算法训练模型时，每次更新权重时，
#为每个权重维护一个影子变量，该影子变量随着训练的进行，
#会最终稳定在一个接近真实权重的值的附近。那么，在进行预测的时候，
#使用影子变量的值替代真实变量的值，可以得到更好的结果
...
def train(mnist):
    ...
    y_ = tf.placeholder(tf.float64, [None, OUTPUT_NODE], name='y-input')
...
#    mnist = input_data.read_data_sets("/root/py/MNIST_data",one_hot=True)
